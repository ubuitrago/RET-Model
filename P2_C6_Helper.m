function PTOT = P2_C4(N,OCI,panelAz) %correct
...
currentTime=0;
T_Panel_Array=zeros(1,289);
for y=totIr_OCI
    currentTime=currentTime+0.0830;
    T_Panel_Array=[T_Panel_Array,T_panel(currentTime,N, y)]; %creates array of the panel temps
end

for i=1:length(totIr_OCI)
    Ptot(i) = Power(totIr_OCI(i),eff,T_Panel_Array(i),len,width,numPanels); 
    Ptot(i) = Ptot(i)/1e6;
end 
 PTOT = Ptot;
