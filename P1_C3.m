...
beta = Tilt; % panel tilt in degrees
panelAz = 46; %panel azimuthal angle in degrees
...
for i=1:length(TOD)
    ...
    anglInc(i) = Incident(Alpha(i),beta(i),solAz(i),panelAz);
end
...
%     global irrD;
    ...
    if beta(i) ~= 0
        irrD(i) = Icd(diffuseTrans(i),Inso,anglInc(i),beta(i)); % clear day diffuse irradation, panel tilt
    elseif beta(i) == 0
        zenith = 90-Alpha(i);
        irrD(i) = Icd(diffuseTrans(i),Inso,zenith,beta(i)); % horizontal surface
    end
